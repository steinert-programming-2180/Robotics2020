...
while(True):
    ...
    filteredImg = gLine.blur_output

    kclose = numpy.ones((3,3), dtype=numpy.uint8)
    kopen = numpy.ones((5,5), dtype=numpy.uint8)
    closing = cv2.morphologyEx(filteredImg, cv2.MORPH_CLOSE, kclose, iterations=2)
    opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, kopen, iterations=6)

    opening = cv2.GaussianBlur(opening, (3,3), 1)
    opening = numpy.float32(opening)
    dst = cv2.cornerHarris(opening, 2, 3, 0.04)
    
    dst = cv2.dilate(dst, None)
    filteredImg[dst>0.01*dst.max()]=[0,0,255]
    ...
    if(len(xCenters) > 0):
        avgX = avgX / len(xCenters)
        avgY = avgY / len(yCenters)

        point = (int(avgX), int(avgY))
        cv2.circle(image, point, 4, (0, 0, 255))
    '''

    # Display the resulting frame
    #cv2.imshow('frame',filteredImg)
    #networkStream.putFrame(filteredImg)
    #print(time.time() - startTime)
